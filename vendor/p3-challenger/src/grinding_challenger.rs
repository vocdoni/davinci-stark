        ...
        + CryptographicPermutation<[<F as Field>::Packing; WIDTH]>,
{
    ...
    fn grind(&mut self, bits: usize) -> Self::Witness {
        ...
        //
        // PATCH (davinci-stark): the original code does `(F::ORDER_U64 as usize).div_ceil(lanes)`.
        // On wasm32, `usize` is 32-bit, so Goldilocks ORDER (2^64-2^32+1) truncates to 1,
        // causing the PoW search to try only 1 candidate and almost certainly fail.
        // Fix: do the arithmetic in u64 first, then clamp to usize::MAX.
        let num_batches = F::ORDER_U64
            .div_ceil(lanes as u64)
